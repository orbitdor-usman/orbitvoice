...
const { app, ipcMain, globalShortcut, systemPreferences, session, screen } = require('electron');
...
const { captureTarget, pasteIntoFocusedField, shutdownInputHelper } = require('./services/textInsertion');
...
handle('speech:enhance', async (_event, { text, id } = {}) => {
  ...
  publishState({ lastTranscript: text });
...
ipcMain.on('widget:recording-state', (event, payload) => {
  ...
  const allowed = new Set(['idle', 'listening', 'processing', 'ai-processing', 'paused', 'permission-required', 'fallback', 'success', 'error']);
